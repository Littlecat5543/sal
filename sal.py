...
def compress_public_key(private_key):
    sk = ecdsa.SigningKey.from_string(bytes.fromhex(private_key), cur>
...
def generate_bitcoin_address(private_key):
    ...
    sha256_hash = hashlib.sha256(bytes.fromhex(compressed_public_key)>
    ripemd160_hash = hashlib.new("ripemd160")
    ripemd160_hash.update(sha256_hash)
    hashed_public_key = ripemd160_hash.digest()
    versioned_hashed_public_key = b"\x00" + hashed_public_key
    checksum = hashlib.sha256(hashlib.sha256(versioned_hashed_public_>
